...
function main()
  ...
  instance_number = parsed_args["instance_number"]
  num_nodes = parsed_args["num_nodes"] * 10000
  approx_num_edges = parsed_args["sparsity"] * num_nodes
  println(parsed_args)
  for i in 1:instance_number
    model = JuMP.Model()
    filename = """$(parsed_args["output_dir"])/pagerank.$(num_nodes).$(approx_num_edges)-$(i).mps.gz"""
